...
var logPath = path.join(__dirname, 'debug.log')
fs.writeFileSync(logPath, '')
var logFile = fs.createWriteSteam()
...
ipc.serve(() => {
  ...
  ipc.server.on('ss:connect', (url, socket) => {
    getSocket(url).then((ws) => {
      ws.on('message', (msg) => {
        logger.info('message from websocket, about to emit to ipc', {msg: msg, url: url})
        ipc.server.emit(socket, 'ws:message', msg)
        logger.info('just emitted message to websocket')
      })
