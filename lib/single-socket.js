...
class SingleSocket extends EventEmitter {
  ...
  connect() {
    this.ipc.connectTo('single-socket', () => {
      ...
      this.ipc.of['single-socket'].on('message', (data) => {
        this.ipc.log('received message from single socket server', data)
      })

      this.ipc.of['single-socket'].on('ws:open', (url) => {
        this.ipc.log(`websocket has been opened on single socket server for ${url}`)
        this.emit('open')
      })

      this.ipc.of['single-socket'].on('ws:close', (url) => {
        this.ipc.log(`websocket has been clsoed on the single socket server for ${url}`)
        this.emit('close')
      })
    })
  }
